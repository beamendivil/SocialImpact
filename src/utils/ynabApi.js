// Handles YNAB transaction loading. Production uses demo data so the deployed
// Stage 1 review build does not expose a personal YNAB token.
...
export async function fetchDemoTransactions() {
  const response = await fetch(`${import.meta.env.BASE_URL}demo-transactions.json`);

  if (!response.ok) {
    throw new Error("Demo transaction data is unavailable");
  }
...
export async function fetchTransactions() {
  if (DEMO_MODE) {
    return fetchDemoTransactions();
  }

  if (!YNAB_API_URL || !YNAB_API_KEY) {
    throw new Error("YNAB API configuration is missing");
  }
