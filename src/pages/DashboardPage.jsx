  ...
  useEffect(() => {
    ...
    async function loadTransactions() {
      try {
        setIsLoading(true);
        setErrorText("");
        const outflowTransactions = (await fetchTransactions())
          .map((transaction) =>
            transaction.milliunits
              ? transaction
              : toImpactTransaction(transaction),
          )
        ...
        try {
          const demoTransactions = await fetchDemoTransactions();
